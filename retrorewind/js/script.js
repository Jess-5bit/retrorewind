 ...
 * Each game loads as game item which consists of a game photo and game title.
...
function loadGenrePage(genre) {
  ...
  genre.forEach(game => {
    ...
    gamePhoto.type = 'button';
    gamePhoto.style.backgroundImage = `url(${game.image})`;
  
    // Create a game information overlay and fill it with the information
    ...
    // Game photos redirect to their game page when clicked on
...
function loadCredits() {
  ...
  Object.keys(games).forEach(genre => {
    ...
    // For each game object in the genre add the credits
    games[genre].forEach(game => {
      const credit = document.createElement('p')
      credit.textContent = `${game.title} @ ${game.credits}`;
...
document.addEventListener("DOMContentLoaded", function () {
  // Load the navigation bar onto each page (Dinh Chang, 2025)
  fetch("navbar.html")
    .then(res => res.text())
    .then(data => {
  ...
  if (window.location.pathname.includes("genre.html")) {
    ...
    // Get "genre" parameter and use it to load the genre
  ...
  if (window.location.pathname.includes("game.html")) {
      ...
      // Prevent the arrow keys and space from scrolling and interfering with the gameplay, only on the game page
